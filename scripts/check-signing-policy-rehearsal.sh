...
if [[ "${1:-}" == "--matrix" ]]; then
  ...
  for name in "${names[@]}"; do
    ...
    if env GITHUB_REF_NAME=v0.8.0 "${base[@]}" "${args[@]}" "$0" >/dev/null 2>&1; then echo "matrix unexpectedly accepted missing $name" >&2; exit 1; fi
  done
...
if [[ "$ref" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  missing=()
  for name in APPLE_CERTIFICATE APPLE_CERTIFICATE_PASSWORD APPLE_SIGNING_IDENTITY APPLE_ID APPLE_PASSWORD APPLE_TEAM_ID WINDOWS_CERTIFICATE_BASE64 WINDOWS_CERTIFICATE_PASSWORD; do
    [[ -n "${!name:-}" ]] || missing+=("$name")
  done
  if ((${#missing[@]})); then
    printf 'Stable releases require platform signing; missing: %s\n' "${missing[*]}" >&2; exit 1
  fi
  echo 'stable=true'
else
  echo 'stable=false'
