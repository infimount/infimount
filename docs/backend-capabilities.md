...
| Google Drive              | Yes               | No                       | No                       | OAuth-backed Google Drive through OpenDAL. Infimount stores OAuth tokens locally; initial authorization codes/refresh tokens must be supplied by the user or a future local OAuth helper. |
| Microsoft OneDrive        | Yes               | No                       | Config-dependent         | OAuth-backed OneDrive Personal through OpenDAL. Enable `versioning` for version listing when the account supports it. Tokens stay local. |
