...
| Google Drive              | Yes               | No                       | No                       | OAuth-backed Google Drive through OpenDAL. The desktop app can guide connection with a local loopback OAuth flow, or advanced users can provide tokens manually. Tokens stay local. |
| Microsoft OneDrive        | Yes               | No                       | Config-dependent         | OAuth-backed OneDrive Personal through OpenDAL. The desktop app can guide connection with a local loopback OAuth flow, or advanced users can provide tokens manually. Enable `versioning` for version listing when the account supports it. Tokens stay local. |
