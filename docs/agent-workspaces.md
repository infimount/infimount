...
For Local Filesystem storage, the configured storage root must be an explicit absolute host path when it is used for an Agent Workspace. Shell notation such as `$HOME/...` is not expanded by the desktop field, and `~/...` is not accepted for namespace binding because workspace identity must be tied to a stable absolute filesystem root. Edit and validate an invalid storage configuration before creating a workspace. Workspace paths such as `/agent-workspaces/example` are then interpreted relative to that configured storage root.
