...
function eligibleWorkspaces(
  ...
  const storageById = new Map(
    storages.map((storage) => [String(storageField<string>(storage, "id", "id") ?? ""), storage]),
  );
  ...
  return workspaces.flatMap((workspace) => {
    ...
    return [{ workspace, storageName: String(storageField<string>(storage, "name", "name") ?? "Local") }];
  });
}
...
export function AgentTaskPrepareDialog({
  ...
  const runPreflight = async () => {
    if (!request || preflightRunning) return;
  ...
  const prepare = async () => {
    if (!request || !preflight || prepareRunning) return;
            ...
            {preflight ? (
              <div className="space-y-3 rounded-xl border border-primary/20 bg-primary/5 p-4" data-testid="agent-task-preflight">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
                  <div><div className="text-xs text-muted-foreground">Selected</div><div>{preflight.selectedItems}</div></div>
                  <div><div className="text-xs text-muted-foreground">Files</div><div>{preflight.fileCount}</div></div>
                  <div><div className="text-xs text-muted-foreground">Folders</div><div>{preflight.directoryCount}</div></div>
                  <div><div className="text-xs text-muted-foreground">Prepared size</div><div>{formatBytes(preflight.totalBytes)}</div></div>
                </div>
                <div className="text-xs text-muted-foreground">
                  The prepared copy will be written to <strong>{preflight.workspaceName}</strong>. Source MCP exposure is {preflight.sourceMcpExposed ? "already enabled independently" : "not enabled"}.
                </div>
        ...
        <DialogFooter>
          ...
          {!prepared && !preflight ? (
            <Button
              ...
              disabled={!request || loadingWorkspaces || preflightRunning}
            >
              {preflightRunning ? "Reviewing…" : "Review access"}
            </Button>
          ) : null}
          {!prepared && preflight ? (
...
export { eligibleWorkspaces, normalizeRequestedOutputs };
